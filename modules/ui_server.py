...
# Point Flask cleanly to our new standalone structure
...
# Copilot Hardening: Suppress verbose background web log spam to optimize frame rates
...
def select_game():
    ...
    try:
        ...
        if selected_game in ["division2", "destiny2", "diablo4"]:
            ...
            print(f"[UI-SERVER] ⚙️ Active profile state updated to: [{SELECTED_GAME.upper()}]")
...
def start_server():
    try:
        # Copilot Hardening: Route on 0.0.0.0 to enable mobile sync, explicitly lock debug mode to off
